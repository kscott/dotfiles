...
Backs up ~/ai to a DMG stored locally in ~/Backups.
- Mounts the DMG, rsyncs ~/ai to current/, saves deleted/changed files to backups/TIMESTAMP/
...
Run every 30 minutes via launchd (com.kenscott.backup-ai.plist).
"""
...
# --- Config ---
HOME = Path.home()
DMG_PATH = HOME / "Backups/ai-folder-backup.dmg"
ICLOUD_BACKUP_PATH = HOME / "Library/Mobile Documents/com~apple~CloudDocs/Backups/ai-folder-backup.dmg"
DMG_SIZE = "500m"
DMG_VOLNAME = "ai-backup"
MOUNT_POINT = Path(f"/Volumes/{DMG_VOLNAME}")
SOURCE = HOME / "ai"
DEST = MOUNT_POINT / "current"
BACKUP_BASE = MOUNT_POINT / "backups"
LOG_FILE = HOME / "logs/backup-ai.log"
LOCK_FILE = Path("/tmp/backup-ai.lock")
BACKUP_TTL_DAYS = 60
LOG_MAX_LINES = 1000
...
def notify_failure(msg):
    ...
    if tn:
        try:
            subprocess.run([
                tn,
                "-title", "AI Backup Failed",
                "-message", body,
                "-sound", "Basso",
                "-ignoreDnD",
                "-group", "ai-backup-failure",
    ...
    try:
        ...
        subprocess.run(
            ["osascript", "-e",
             f'display notification "{safe}" with title "AI Backup Failed" sound name "Basso"'],
            capture_output=True, timeout=15,
        )
    except Exception:
        pass
